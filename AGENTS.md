...
install with `python -m pip install -e ".[test,lint,typecheck]"` for
contributor test/lint/type-check runs (or
`python -m pip install -e .` for runtime-only installs). Run
`python -m ruff check .`, run `python -m mypy sm_logtool`, run
